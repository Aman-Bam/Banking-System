...
const Dashboard = () => {
    ...
    const fetchAccounts = async () => {
        try {
            ...
            const allData = await accountApi.getAllAccounts();
            setAllRegisteredAccounts(allData.accounts || []);
        } catch (error) {
            ...
            <div className="space-y-8">
                ...
                >
                    <div className="space-y-6">
                        <p className="text-white/70">
                            Are you sure you want to open a new account? It will be activated immediately.
                        </p>
                ...
                >
                    <form onSubmit={handleTransfer} className="space-y-4">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-white/80 mb-1 ml-1">From Account (Your Account)</label>
                            <select
                            ...
                            >
                                {accounts.map(acc => (
                                    <option key={acc._id} value={acc._id} className="bg-slate-800">
                                        {acc.userName || acc.user?.name || 'Account'} (${acc.balance}) - ...{acc._id.slice(-6)}
                                    </option>
                                ))}
                            </select>
                        </div>
                        ...
                        <div>
                            <label className="block text-sm font-medium text-white/80 mb-1 ml-1">To Account (Select Recipient)</label>
                            <select
                                value={toAccountId}
                                ...
                                required
                            >
                                <option value="" className="bg-slate-800">Select Recipient Account</option>
                                {allRegisteredAccounts.filter(acc => acc._id !== selectedFromAccount).map(acc => (
                                    <option key={acc._id} value={acc._id} className="bg-slate-800">
                                        {acc.userName || acc.user?.name || 'User'} ({acc.user?.email || 'Registered'}) - ...{acc._id.slice(-6)}
                                    </option>
                                ))}
                            </select>
                        </div>
